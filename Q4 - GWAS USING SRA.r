...
# HOMEWORK - HS 541 Plant Breeding Methods - 4RD QUESTION
...
head(Y)
myY <- Y[,c(1, 3)] # dBLUP utilizado com a caracteristica SRA aplicada.
head(myY)
...
#O QQ-plot mostra que o modelo está bem calibrado, sem inflação excessiva de p-valores.

#O Manhattan em forma de circos revela regiões específicas no genoma associadas à SRA, algumas delas consistentes entre diferentes métodos de correção.

#Isso indica que tem SNPs candidatos confiáveis, possivelmente ligados a genes que influenciam essa característica.
