...
# HOMEWORK - HS 541 Plant Breeding Methods - 3TH QUESTION
...
solF <- remlf90(fixed = SRAadj ~ rep + N + gid, 
                random = ~ GN,
                method = "em",
                data = pheno) 
...
## model A (pedigree)
modA <- remlf90(fixed = SRAadj ~ N + rep, 
                random = ~ 1, 
...
## model Ga
modGa <- remlf90(fixed = SRAadj ~ N + rep, 
                      random = ~ 1, 
...
## model Ga & Gd
modGaGd <- remlf90(fixed = SRAadj ~ N + rep, 
                       random = ~ 1,
                       generic = list(AD = list(Za, Ga),
...
## model
NCII <- remlf90(fixed = SRAadj ~ N + rep, 
                   random = ~ 1,
...
FD <- remlf90(fixed = SRAadj ~ N + rep,  #usando a variavel SRA
                random = ~ 1,
...
fviz_cluster(k, data = scale(d), main = "Heterotic Pools - SRA")
k$size
sum(k$size)
