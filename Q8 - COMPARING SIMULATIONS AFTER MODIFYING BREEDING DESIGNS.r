###################################################
#HOMEWORK - HS 541 Plant Breeding Methods - 8TH QUESTION
...
# crossing block
n.parents <- 60 #aumetando para 60
n.crosses <- 20 #reduzindo para 20
progenie.size <- 200 #aumentando para 200
...
###  indica que a seleção baseada no fenótipo será altamente eficaz para aprimorar o valor genético dessa característica. ###
...
# o método de melhoramento tradicional, mesmo sem a seleção genômica, é capaz de gerar ganho genético ao longo das gerações ##
...
#Um valor de correlação de aproximadamente 0.68 é considerado uma acurácia moderada.

#Sugere que o  modelo de seleção genômica é razoavelmente bom em prever o valor genético real dos indivíduos. 
#Em outras palavras, a seleção baseada no modelo genômico será mais eficiente do que a seleção aleatória, 
#mas ainda haverá espaço para melhorias.
...
# Primeira simulação (SEM ALTERAR O SCRIPT ORIGINAL):

# - Trad apresentou resposta baixa (~4.3), com pouca variabilidade.
# - Trad+GS foi superior (~5.2–5.5), com ganho mais estável.
# - Conclusão: inclusão de GS aumenta o ganho de forma consistente.

# Segunda simulação (ALTERANDO O NUMERO DE PARENTAIS, CROSSES E TAMANHO DA PROGÊNIE):

# - Trad subiu (~5.2), reduzindo a diferença para Trad+GS.
# - Trad+GS também subiu (~5.7–6.0), mas com maior variabilidade.
# - Alterações no código favoreceram o acréscimo no Trad 
#   (provavelmente herdabilidade, intensidade de seleção ou ciclos).
# - Conclusão: GS continua melhor, mas a magnitude do ganho depende dos parâmetros.
############################# the end #########################################
